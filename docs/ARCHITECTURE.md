...
| 16 | Unumdorlik (moliya va kadrlar hajmi) | `db:perf-seed:finance` — mavjud perf bazasi ustiga 24 oylik moliya/kadrlar hajmi (60 000 to‘lov, 12 400 xarajat, 3 000 tushum, 600 qaytarish, 400 o‘tkazma, 25 xodim va maosh davrlari, 150 000 audit, 3 000 alert, 600 hujjat; kassa qoldiqlari daftarga mos). `perf:bench` — og‘ir servislarni to‘g‘ridan-to‘g‘ri o‘lchaydi (qizdirish + 5 takror, median, JSON natija). Foyda-zarar va pul oqimi grafigi davrdagi barcha daftar yozuvlarini JS'ga yuklash o‘rniga bazada kun/oy × modda bo‘yicha yig‘adi (`OPERATING_LEDGER_SQL` — `OPERATING_LEDGER_WHERE` ning SQL ko‘rinishi, kalit `date` turida). Faoliyat lentasi uchun `createdAt` indekslari: xarajat, davomat sessiyasi, uy vazifasi, imtihon (qo‘shimcha migratsiya). Natijalar — README «Unumdorlik sinovi» | ✅ |
| 17 | Testlar | **E2E (Playwright)**: `npm run test:e2e` alohida `crm_e2e` bazasini qayta yaratib seed qiladi, backend va frontendni alohida portlarda ishga tushiradi; ssenariylar — noto‘g‘ri parol, himoyalangan sahifaga qaytish va sahifa yangilanganda sessiya tiklanishi, chiqish, o‘qituvchi/buxgalter menyu va sahifa ruxsatlari, lead qo‘shish va statusini o‘zgartirish, qarzdordan to‘lov (qarz kamayishi API orqali), xarajat qo‘shish, foyda-zarar va direktor paneli, qorong‘i mavzu, telefonda davomat (gorizontal siljish yo‘q); har testda brauzer konsol xatosi testni yiqitadi; CI'da alohida `e2e` job (xatoda hisobot va trace artefakt). **Backend**: qarzdorlik va follow-up eslatma joblari (dedupe, o‘quv markaz kuni, faqat ruxsatli faol xodim), forma ma’lumotnomalari, imtihon kirish/validatsiya/filtrlari, markaziy xato ushlagich (tafsilot foydalanuvchiga chiqmasligi). **Topilgan va tuzatilgan xato**: `.partial()` ichida `.default()` ishlagani uchun qisman tahrirda yuborilmagan maydon jimgina standart qiymatga qaytardi — imtihon maksimal bali/XP/holati, uy vazifasi bali/XP/holati, davomat sessiyasi holati, kassa tartibi, takroriy xarajat to‘lov usuli; standart qiymatlar faqat yaratish sxemalarida, barcha `update*Schema`larni avtomatik tekshiruvchi unit test qo‘shildi (qo‘ng‘iroq tahriri — ataylab to‘liq almashtirish) | ✅ |
| T1 | Takroriy to‘lovdan himoya | `payments.idempotencyKey` (unique, qo‘shimcha migratsiya): to‘lov formasi har ochilishda kalit yaratadi — ikki marta bosish yoki tarmoq qayta urinishi yangi to‘lov yaratmaydi, server mavjud kvitansiyani 200 bilan qaytaradi (kalit boshqa o‘quvchi/summa bilan ishlatilsa 409). Bitta o‘quvchiga parallel to‘lovlar qarz qatorini `FOR UPDATE` bilan qulflab ketma-ket bajariladi, qarzdan oshish tekshiruvi tranzaksiya ichida (avval tashqarida edi — ikki parallel so‘rov ikkalasi o‘tib ketishi mumkin edi). Oxirgi 10 daqiqada shu o‘quvchidan xuddi shu summa xuddi shu to‘lov kuni bilan qabul qilingan bo‘lsa (bekor qilinganlar va boshqa kunga kiritilgan qismlar hisobga olinmaydi) 409 `duplicatePayment` — formada ogohlantirish, «Baribir saqlash» (`confirmDuplicate`, auditda `duplicateOf`) yoki «Saqlamasdan yopish» | ✅ |
| T2 | To‘lov jadvali va muddatlar | `PaymentInstallment` (tartib, muddat, summa, izoh; qo‘shimcha migratsiya, summa > 0 CHECK). Qism holati saqlanmaydi: `Debt.paidAmount` (qaytarishlar ayirilgan) qismlarga muddat tartibida taqsimlanadi — to‘langan / qisman / bugun / kutilmoqda / muddati o‘tgan (kun bilan), shuning uchun to‘lovni bekor qilish yoki qaytarish jadvalga o‘zi ta’sir qiladi. Yangi o‘quvchi va leaddan aylantirishda standart jadval: kurs davomiyligi bo‘yicha oylik, 1 000 so‘mga yuvarlangan, farq oxirgi qismga, oy oxiri to‘g‘ri (31-yanvar → 28-fevral). `GET /students/:id/payment-schedule` (`debt.view` yoki `payment.view`), `POST …/generate` va `PUT …` (`payment.create`; qismlar yig‘indisi shartnomaga teng, muddatlar o‘sib boruvchi, auditga yoziladi); shartnoma narxi keyin o‘zgarsa `mismatch`. Qarzdorlar: `?due=overdue|upcoming` filtri, har qatorda kechikish kuni/summasi yoki keyingi muddat, xulosada muddati o‘tgan va 7 kunlik summa — hisob bitta SQL (window function). `PAYMENT_OVERDUE` alerti (sozlanadi, standart 3 kun; 30 kundan ortiq — yuqori muhimlik). Mavjud o‘quvchilar uchun `npm run db:backfill-schedules` (faqat jadvali yo‘qlarga, takror ishga tushirish xavfsiz). Frontend: o‘quvchi profilida «To‘lov jadvali» (qayta tuzish, qo‘lda tahrirlash), qarzdorlik sahifasida muddat filtri va «Jadval» ustuni | ✅ |
