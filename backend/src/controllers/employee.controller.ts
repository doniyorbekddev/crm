...
/** Maosh summalari faqat salary.view egasiga */
async function canViewSalary(req: Request): Promise<boolean> {
...
export const employeeController = {
  async list(req: Request, res: Response): Promise<void> {
    ...
    const { items, total } = await employeeService.list(query, await canViewSalary(req));
  ...
  async getById(req: Request, res: Response): Promise<void> {
    ...
    sendSuccess(res, await employeeService.getById(id, await canViewSalary(req)));
  },
